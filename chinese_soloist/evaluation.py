...
def eval_all(predicts, answers):
    ...
    # redial_bleu = eval_redial_bleu(predicts, answers)
    # log_dict.update(redial_bleu)
    # soloist_bleu = eval_soloist_bleu(predicts, answers)
    # log_dict.update(soloist_bleu)
    # distinct = eval_distinct(predicts, answers)
    # log_dict.update(distinct)
    redial_dist = eval_redial_dist(predicts, answers)
    log_dict.update(redial_dist)
    return log_dict
...
def _eval_rouge(run, ref):
    ...
    return {'ROUGE_1': rounder(rouge_1), 'ROUGE_2': rounder(rouge_2), 'ROUGE_L': rounder(rouge_l)}
...
def eval_bleu(predicts, answers):
    ...
    for answer in answers:
        answer = answer.split('\t')
        _answers.append([' '.join(jieba.cut(item)).split() for item in answer])
    bleu1 = 0.
    bleu2 = 0.
    bleu3 = 0.
    bleu4 = 0.
    ...
    for predict, answer in zip(predicts, _answers):  # run_dict是example的数量
        ...
        predict = ' '.join(jieba.cut(predict)).split()
...
def eval_soloist_bleu(predicts, answers):
    bscorer = BLEUScorer()
    blue_score = bscorer.score(predicts, answers)
    return {'solo': rounder(blue_score * 100)}


def redial_bleu(guess, answers):
...
def eval_redial_bleu(predicts, answers):
    score = []
    for predict, answer in zip(predicts, answers):
        score.append(redial_bleu(predict, [answer]))
    return {'ReBleu': rounder(sum(score) / len(score) * 100)}
...
def distinct_n_grams(tokenized_lines, n):
    n_grams_all = set()
    for line in tqdm(tokenized_lines, desc=f"distinct_n_grams{n}"):
...
def eval_redial_dist(predicts, answers):
    predicts = [' '.join(jieba.cut(item)).split() for item in predicts]
