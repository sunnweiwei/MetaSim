...
class MultiWozDB(object):
    ...
    hospital_db = json.load(open('db/hospital_db.json', 'r'))
    all_items = {}
    # for domain in domains:
...
class MultiWozEvaluator(BaseEvaluator):
    ...
    def _evaluateGeneratedDialogue(self, dialog, goal, realDialogue, real_requestables, soft_acc=False):
        #self._evaluateGeneratedDialogue((dial,bs_state), goal, data, requestables, soft_acc=mode =='soft')
        ...
        real_state = {}
        for domain in goal.keys():
            real_state[domain] = goal[domain]['informable']
    ...
    def _evaluateRealDialogue(self, dialog, filename):
        ...
        goal:
        {'restaurant': {'informable': {'food': 'italian', 'pricerange': 'cheap', 'area': 'centre'},
                        'requestable': ['reference'],
                        'booking': {'people': '6', 'day': 'sunday', 'invalid': False, 'time': '18:45'}},
         'hotel': {'informable': {'pricerange': 'cheap', 'internet': 'yes', 'type': 'guesthouse', 'parking': 'yes'},
                   'requestable': ['reference'],
                   'booking': {'people': '6', 'day': 'sunday', 'invalid': False, 'stay': '4'}}}

        success: 0/1(if all requestable is offered)
        match: 0/1(if all info can find the true venue)

        real_requestables {'restaurant': ['reference'],
                           'hotel': ['reference']}

        stats {'restaurant': [0, 0, 0], 'hotel': [0, 0, 0], 'attraction': [0, 0, 0], 'train': [0, 0, 0],
                         'taxi': [0, 0, 0],
                         'hospital': [0, 0, 0], 'police': [0, 0, 0]}
        [match, success, have_domin]"""
        ...
        for t in range(len(m_targetutt)):
            for domain in domains_in_goal:
                ...
                if domain in ['restaurant', 'hotel', 'attraction', 'train']:
                    ...
                    # find the true venues currently
        ...
        for domain in goal.keys():
            ...
            # print(domain + ': ', venue_offered[domain])
        ...
        real_state = {}
        for domain in goal.keys():
            real_state[domain] = dialog['goal'][domain]['info']

        gen_state = {}
        for domain in goal.keys():
            gen_state[domain] = dialog['log'][len(m_targetutt) * 2 - 1]['metadata'][domain]['semi']
    ...
    def evaluateModel(self, dialogues, dialogues_bs, real_dialogues=False, mode='valid'):
        ...
        real_successes, real_matches = 0, 0
        total = 0
        ...
        for filename, dial in tqdm(dialogues.items()):
            ...
            success, match, stats = self._evaluateGeneratedDialogue((dial,bs_state), goal, data, requestables,
                                                                    soft_acc=mode =='soft')
            # print('generation', match, success)
        ...
        report = ""
        report += '{} Real Matches : {:2.2f}%'.format(mode, (real_matches / float(total) * 100)) + "\n"
        report += '{} Real Success : {:2.2f}%'.format(mode, (real_successes / float(total) * 100)) + "\n"
...
from parse import parse_decoding_results
import argparse
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--eval_file", default='output/all_1new.json', type=str, help="The input evaluation file.")
    ...
    f = args.eval_file
    output_file = ['data-10_context-15']
    for f in output_file:
        file = 'output_new/' + f + '.json'
        res, res_bs = parse_decoding_results(file, mode)
        # res_bs = json.load(open('output_new/' + f + '_bs.json', 'r'))

        # PROVIDE HERE YOUR GENERATED DIALOGUES INSTEAD
        evaluator.evaluateModel(res, res_bs, True, mode=mode)
