...
system_path = 'system_redial/'
flag = True
system_assign = {
    'redial-context-15': [i for i in range(4)] + [i for i in range(28, 32)],
    'redial-context-3': [i for i in range(4, 8)],
    'redial-context-1': [i for i in range(8, 12)],
    'redial-domain-3': [i for i in range(12, 16)],
    'redial-domain-1': [i for i in range(16, 20)],
    'redial-recommend-3': [i for i in range(20, 24)],
    'redial-recommend-1': [i for i in range(24, 28)],
...
system_assign_index = {
    ...
    'redial-context-3': 0,
    'redial-context-1': 0,
    'redial-recommend-3': 0,
    'redial-recommend-1': 0,
    'redial-domain-3': 0,
    'redial-domain-1': 0,
    # 'redial-context-15': 0,
    # 'JDDC-context-15': 0,
...
def generate_response():
    ...
    if system_type.split('-')[0] == 'mwoz':
    ...
    elif system_type.split('-')[0] == 'redial':
        context = request.form['context']
        context = context.split('[SEP]')
        # turn = context[-1]
        # logger.info(turn)
        ...
        if 'index' not in request.form.keys():
            ...
            def trunc_encode(encode_text, max_length, label_flag=False):
                encode_text['input_ids'] = encode_text['input_ids'][-max_length:]
                encode_text['attention_mask'] = encode_text['attention_mask'][-max_length:]
                if label_flag:
                    labels = copy.copy(encode_text['input_ids'])
                else:
                    labels = [-100] * len(encode_text['input_ids'])
                return encode_text, labels

            source = context + '=>'
            encode = tokenizer_redial.encode_plus(
                source,
                truncation=False,
            )
            encode, _ = trunc_encode(encode, 480)
            logger.error(encode)
            context_tokens = encode['input_ids']
            data = {
                'context_tokens': context_tokens,
            }
            logger.error(f"input_data {data}")
        ...
        if 'index' in request.form.keys():
            ...
            dialog_data['task'] = request.form['task']
            turn_now = total[task_type]
            write_pkl(dialog_data, f'output/interaction_redial/{system_type}_{task_type}_{turn_now}.json')
            total[task_type] += 1
            total_dialog[task_type] += dialog_data['turn']
            for k in total.keys():
                logging_txt = 'task: ' + k + ' turn: ' + str(int(total[k])) + \
